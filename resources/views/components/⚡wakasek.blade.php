...
<style>
    ...
    .wakasek-dashboard .welcome-banner > * { position: relative; z-index: 1; }
    ...
    .wakasek-page-header > * { position: relative; z-index: 1; }

    .wakasek-content-card { overflow: hidden; box-shadow: 0 6px 20px rgba(22, 33, 62, .045); }
    ...
    .wakasek-table tbody tr:hover { background: #f8fbff; }

    .rekap-card { border-top: 3px solid var(--rekap-color, #2563eb); box-shadow: 0 7px 22px rgba(22, 33, 62, .05); }

    .rekap-card .card-header-custom { background: linear-gradient(100deg, var(--rekap-tint, #eff6ff), #fff 75%); }
    ...
    .rekap-card .rekap-toolbar { width: 100%; }
    .rekap-card .rekap-toolbar select { min-width: 145px; }
    .rekap-card .rekap-list { padding: 14px; }
    ...
    .rekap-card .rekap-row + .rekap-row { margin-top: 8px; }
    .rekap-card .rekap-label { display: none; }
    ...
    .rekap-choice:hover, .rekap-choice:focus-visible {
        transform: translateY(-3px);
        border-color: #8bb5f5;
    ...
    @media (max-width: 767.98px) {
        .wakasek-page-header { padding: 20px; }
        .rekap-card .card-header-custom { align-items: flex-start !important; }
        .rekap-card .rekap-toolbar { display: grid !important; grid-template-columns: 1fr 1fr; }
        .rekap-card .rekap-toolbar select { grid-column: 1 / -1; width: 100%; }
        .rekap-card .rekap-toolbar .btn { width: 100%; white-space: nowrap; }
        .rekap-card .rekap-list { padding: 10px; }
        .rekap-card .rekap-row { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px 12px; padding: 12px; }
        .rekap-card .rekap-cell { min-width: 0; overflow-wrap: anywhere; }
        .rekap-card .rekap-cell:first-child { grid-column: 1 / -1; }
        .rekap-card .rekap-label { display: block; color: #71809a; font-size: 10px; font-weight: 700; letter-spacing: .04em; text-transform: uppercase; }
    }
    ...
    @media print {
        ...
        .print-report .print-meta { margin-bottom: 18px; color: #52627d; font-size: 9pt; }
...
>
    @if ($this->dataCetak)
        <div class="print-report" style="display:block;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;border-bottom:3px solid #2563eb;padding-bottom:14px;margin-bottom:14px;">
                <div><div style="font-size:9pt;letter-spacing:.12em;color:#52627d;font-weight:700;">SIJAGA · LAPORAN SEKOLAH</div><h1 style="margin-top:6px;">{{ $this->dataCetak['judul'] }}</h1><div class="print-meta">Periode: {{ $this->dataCetak['periode'] }} · Dicetak: {{ now('Asia/Jakarta')->format('d/m/Y H:i') }}</div></div>
                <button type="button" class="btn btn-primary" onclick="window.print()">Cetak</button>
            </div>
            <table><thead><tr>@foreach ($this->dataCetak['kolom'] as $kolom)<th>{{ $kolom }}</th>@endforeach</tr></thead><tbody>
                @forelse ($this->dataCetak['data'] as $baris)<tr>@foreach ($baris as $nilai)<td>{{ $nilai }}</td>@endforeach</tr>@empty<tr><td colspan="{{ count($this->dataCetak['kolom']) }}">Tidak ada data pada periode ini.</td></tr>@endforelse
            </tbody></table>
        </div>
        <script>
    ...
    <section x-cloak x-show="activeSection === 'dashboard'" :class="{ 'd-none': activeSection !== 'dashboard' }" wire:poll.60s id="wakasek-dashboard" class="wakasek-dashboard">
        <div class="welcome-banner mb-4">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                <div>
                    <div class="text-uppercase fw-bold" style="font-size:11px; letter-spacing:.14em; color:rgba(255,255,255,.72);">SIJAGA · PANEL PIMPINAN</div>
    ...
    <section x-cloak x-show="activeSection === 'rekap'" :class="{ 'd-none': activeSection !== 'rekap' }" id="rekap" class="d-grid gap-4">
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
            <button type="button" class="rekap-back" wire:click="tutupRekap">← Kembali ke semua rekap</button>
            <span class="badge rounded-pill px-3 py-2" style="background:#dbeafe;color:#1d4ed8;">TAMPILAN REKAP</span>
        </div>
        @endif
        ...
        <section class="card-custom rekap-card" id="rekap-jurnal" style="--rekap-color:#2563eb;--rekap-tint:#eff6ff;">
            <div class="card-header-custom d-flex flex-wrap justify-content-between align-items-center gap-3">
                ...
                <div class="rekap-toolbar d-flex flex-wrap justify-content-end gap-2">
                    ...
                    <button class="btn btn-sm btn-outline-primary" type="button" wire:click="exportCsv('jurnal')">Ekspor Excel</button>
                    <a class="btn btn-sm btn-primary" target="_blank" href="{{ $this->urlCetak('jurnal') }}">Cetak / Print</a>
                </div>
            </div>
            <div class="rekap-list d-grid gap-2">
                @forelse ($this->riwayatJurnal as $jurnal)
                    <article class="rekap-row" wire:key="rekap-jurnal-{{ $jurnal->id_jurnal }}">
                        <div class="rekap-cell"><span class="rekap-label">Tanggal</span>{{ $jurnal->tanggal?->format('d/m/Y') }}</div>
                        <div class="rekap-cell"><span class="rekap-label">Guru</span><strong>{{ $jurnal->guru?->nama ?? '-' }}</strong></div>
                        <div class="rekap-cell"><span class="rekap-label">Kelas</span>{{ $jurnal->kelas?->nama_kelas ?? '-' }}</div>
                        <div class="rekap-cell"><span class="rekap-label">Jam</span>Ke-{{ $jurnal->jam_ke }}</div>
                        <div class="rekap-cell"><span class="rekap-label">Status</span><span class="badge {{ $jurnal->status_validasi === 'Divalidasi' ? 'bg-success' : ($jurnal->status_validasi === 'Ditolak' ? 'bg-danger' : 'bg-warning text-dark') }}">{{ $jurnal->status_validasi }}</span></div>
                        <div class="rekap-cell" style="grid-column:1/-1;"><span class="rekap-label">Materi</span>{{ $jurnal->materi }} · Hadir {{ $jurnal->jumlah_hadir ?? '-' }}, tidak hadir {{ $jurnal->jumlah_tidak_hadir ?? '-' }} · {{ $jurnal->status_kehadiran_guru }}</div>
                    </article>
                @empty<div class="text-center text-muted py-4">Tidak ada jurnal untuk filter ini.</div>@endforelse
            </div>
        </section>
        @endif
        ...
        <section class="card-custom rekap-card" id="rekap-belum-mengisi" style="--rekap-color:#2563eb;--rekap-tint:#eff6ff;">
            <div class="card-header-custom d-flex flex-wrap justify-content-between align-items-center gap-3">
                ...
                <div class="rekap-toolbar d-flex flex-wrap justify-content-end gap-2">
                    ...
                    <a class="btn btn-sm btn-primary" target="_blank" href="{{ $this->urlCetak('belum-mengisi') }}">Cetak / Print</a>
                </div>
            </div>
            <div class="rekap-list d-grid gap-2">
                @forelse ($this->rekapBelumMengisi as $jadwal)
                    <article class="rekap-row" wire:key="rekap-belum-{{ $jadwal->id_jadwal }}-{{ $jadwal->tanggal_rekap->format('Ymd') }}">
                        <div class="rekap-cell"><span class="rekap-label">Tanggal</span>{{ $jadwal->tanggal_rekap->format('d/m/Y') }}</div><div class="rekap-cell"><span class="rekap-label">Guru</span><strong>{{ $jadwal->guru?->nama ?? '-' }}</strong></div><div class="rekap-cell"><span class="rekap-label">Kelas</span>{{ $jadwal->kelas?->nama_kelas ?? '-' }}</div><div class="rekap-cell"><span class="rekap-label">Jam</span>Ke-{{ $jadwal->jam_ke }}</div><div class="rekap-cell"><span class="rekap-label">Waktu</span>{{ substr($jadwal->jam_mulai, 0, 5) }}–{{ substr($jadwal->jam_selesai, 0, 5) }}</div>
                    </article>
                @empty<div class="text-center text-muted py-4">Tidak ada jadwal tanpa jurnal untuk filter ini.</div>@endforelse
            </div>
        </section>
        @endif
        ...
        <section class="card-custom rekap-card" id="rekap-kehadiran" style="--rekap-color:#2563eb;--rekap-tint:#eff6ff;">
            <div class="card-header-custom d-flex flex-wrap justify-content-between align-items-center gap-3">
                ...
                <div class="rekap-toolbar d-flex flex-wrap justify-content-end gap-2">
                    ...
                    <a class="btn btn-sm btn-primary" target="_blank" href="{{ $this->urlCetak('kehadiran') }}">Cetak / Print</a>
                </div>
            </div>
            <div class="rekap-list d-grid gap-2">
                @forelse ($this->rekapKehadiran as $jadwal)
                    <article class="rekap-row" wire:key="rekap-hadir-{{ $jadwal->id_jadwal }}-{{ $jadwal->tanggal_rekap->format('Ymd') }}">
                        <div class="rekap-cell"><span class="rekap-label">Tanggal</span>{{ $jadwal->tanggal_rekap->format('d/m/Y') }}</div><div class="rekap-cell"><span class="rekap-label">Guru</span><strong>{{ $jadwal->guru?->nama ?? '-' }}</strong></div><div class="rekap-cell"><span class="rekap-label">Kelas</span>{{ $jadwal->kelas?->nama_kelas ?? '-' }}</div><div class="rekap-cell"><span class="rekap-label">Jam</span>Ke-{{ $jadwal->jam_ke }}</div><div class="rekap-cell"><span class="rekap-label">Status</span><span class="badge {{ $jadwal->status_kehadiran === 'Hadir' ? 'bg-success' : ($jadwal->status_kehadiran === 'Tanpa Keterangan' ? 'bg-danger' : 'bg-warning text-dark') }}">{{ $jadwal->status_kehadiran }}</span></div>
                    </article>
                @empty<div class="text-center text-muted py-4">Tidak ada data kehadiran untuk filter ini.</div>@endforelse
            </div>
        </section>
        @endif
        ...
        <section class="card-custom rekap-card" id="rekap-tanpa-keterangan" style="--rekap-color:#2563eb;--rekap-tint:#eff6ff;">
            <div class="card-header-custom d-flex flex-wrap justify-content-between align-items-center gap-3">
                ...
                <div class="rekap-toolbar d-flex flex-wrap justify-content-end gap-2">
                    ...
                    <a class="btn btn-sm btn-primary" target="_blank" href="{{ $this->urlCetak('tanpa-keterangan') }}">Cetak / Print</a>
                </div>
            </div>
            <div class="rekap-list d-grid gap-2">
                @forelse ($this->rekapTanpaKeterangan as $jadwal)
                    <article class="rekap-row" wire:key="rekap-tanpa-{{ $jadwal->id_jadwal }}-{{ $jadwal->tanggal_rekap->format('Ymd') }}">
                        <div class="rekap-cell"><span class="rekap-label">Tanggal</span>{{ $jadwal->tanggal_rekap->format('d/m/Y') }}</div><div class="rekap-cell"><span class="rekap-label">Guru</span><strong>{{ $jadwal->guru?->nama ?? '-' }}</strong></div><div class="rekap-cell"><span class="rekap-label">Kelas</span>{{ $jadwal->kelas?->nama_kelas ?? '-' }}</div><div class="rekap-cell"><span class="rekap-label">Jam</span>Ke-{{ $jadwal->jam_ke }}</div><div class="rekap-cell"><span class="rekap-label">Status</span><span class="badge bg-danger">{{ $jadwal->status_kehadiran }}</span></div>
                    </article>
                @empty<div class="text-center text-muted py-4">Tidak ada guru tanpa keterangan untuk filter ini.</div>@endforelse
            </div>
        </section>
        @endif
    </section>
