...
new class extends Component
{
    ...
    public function getBolehInputPiketProperty(): bool
    {
        return session('role') === 'guru'
            && app(GuruPiketAccessService::class)->bertugasHariIni((int) session('id_pengguna'));
    }
...
<div wire:poll.30s>
    ...
    <div class="alert alert-warning mb-4" role="status">
        Anda tidak memiliki hak akses piket hari ini.
    </div>
    @endunless

    <div class="alert alert-info mb-4">
        Guru piket fokus mendistribusikan tugas titipan/izin luar kelas. Validasi kehadiran guru berjalan otomatis oleh
        sistem melalui jurnal.
    </div>
    ...
    <div class="card-custom mb-4 border border-danger">
        ...
        <div class="p-3 text-muted small">
            Hubungi guru terkait untuk memastikan penyampaian tugas atau kondisi kelas.
        </div>
        ...
        <div class="p-3 border-top d-flex flex-wrap justify-content-between gap-2"
            ...
            <div>
                ...
                <div class="text-muted small">
                    {{ $jadwal->mapel_diampu ?: '-' }} · {{ $jadwal->kelas?->nama_kelas ?: '-' }}
                </div>
            </div>
            <div class="text-danger fw-semibold">
                Jam ke-{{ $jadwal->jam_ke }} · belum ada jurnal
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div class="card-custom p-4 d-flex flex-wrap align-items-center justify-content-between gap-3">
        <div>
            ...
            <div class="text-muted small">
                {{ $this->statusKehadiranGuru->count() }} jadwal hari ini · nama guru urut A–Z
            </div>
        </div>
    ...
    <div class="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center p-3"
        style="z-index: 1060; background: rgba(15, 23, 42, .58);" wire:click.self="closeDaftarJadwal"
        wire:keydown.escape.window="closeDaftarJadwal">

        <section class="card-custom w-100 overflow-hidden" style="max-width: 1000px; max-height: 90vh;" role="dialog"
            aria-modal="true" aria-labelledby="piket-jadwal-title">
                ...
                <table class="table table-hover mb-0 align-middle">
                    ...
                    <tbody>
                        ...
                        <tr wire:key="jadwal-{{ $jadwal->id_jadwal }}">
                            ...
                            <td>
                                <span @class([ 'badge' , 'bg-success'=> $jadwal->status_sistem === 'Hadir (jurnal
                                    diisi)',
